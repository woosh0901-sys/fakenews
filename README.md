...
![Fake News Defender 시스템 아키텍처](docs/architecture.svg)

검증 요청은 Vercel 서버리스 함수의 FastAPI로 들어가 네 단계 파이프라인을 거칩니다. 파란 점선은 단축 경로로, 같은 URL에 대해 24시간 안에 `REAL` 판정 캐시가 있으면 1~4단계와 모든 외부 API 호출을 건너뛰고 즉시 응답합니다. 주황색으로 표시한 두 지점은 판정을 강행하지 않고 `SUSPICIOUS`로 유보하는 저하 경로입니다.

`/api/history`는 최신 25건만 조회하고 `/api/stats`와 `/api/stats/rankings`는 전체 행을 집계합니다. 히스토리 목록 길이를 제한하면서도 누적 통계는 정확하게 유지하기 위한 구분입니다.
