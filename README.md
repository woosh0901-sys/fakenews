...
4. **비정형 SNS/커뮤니티 루머의 검증 불가능**: 인스타그램 릴스 캡션, 커뮤니티 글은 비격식적 구어체로 작성되어 기존 키워드 매칭률이 극도로 떨어집니다.
...
* **본문 기반 중복 검출 및 대립 방향성 감지 (`are_articles_duplicated`)**: 
  ...
  - 방향성 대립 단어(투자 vs 철회, 찬성 vs 반대, 사실무근 vs 사실 등) 감지 시 제목이 유사해도 중복에서 배제합니다.
...
├── fact_checker_by_url.py       # RAG 파이프라인, 본문 중복 검출, 출처 선별, LLM 교차 검증
...
├── test_rag_pipeline.py         # 본문 중복, 대립 방향성, 1차자료 우선, 근거품질 단위 테스트 (12개)
...
총 **24개 단위 및 통합 테스트**로 파이프라인 무결성을 상시 검증합니다:
...
python test_rag_pipeline.py   # 본문 중복, 대립 방향성, 1차자료 우선, 근거품질 산출 등 12개 테스트
